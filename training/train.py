...
parser.add_argument('--max-iter', default=0, type=int,
                    metavar='N', help='maximum iterations per epoch (default: 0, not limited)')
...
def main():
    ...
    # load data
    normalize = transforms.Normalize(mean=VGG_M_2048.INPUT_MEAN,
                                     std=VGG_M_2048.INPUT_STD)
    train_sampler = None
    ...
    # create model
    model = VGG_M_2048(num_classes=imagenet.num_classes,
                       model_path='models/imagenet_vgg_m_2048.mat',
                       class_names=imagenet.synsets,
                       class_desc=imagenet.synset_desc)
    print(model)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, optimizer, epoch, args.max_iter,
        ...
        if epoch % args.eval_freq == 0 or epoch == args.epochs - 1:
            # remember lowest total loss and save checkpoint
            loss = validate(val_loader, model, smoothness_criterion, bbox_criterion, cls_criterion, epoch, logger)
...
def train(train_loader: torch.utils.data.DataLoader,
    ...
    for iter, (target, pos_sample, neg_sample, cid, bbox, pos_bbox) in enumerate(train_loader):
        ...
        cls_losses.update(cls_loss.item(), target.size(0))
        ...
        if iter % args.vis_freq == 0:
            ...
            for c in range(target.shape[1]):
                concat_imgs[:, c, :, :target.shape[3]] = target[:, c, ...] * VGG_M_2048.INPUT_STD[c]\
                                                         + VGG_M_2048.INPUT_MEAN[c]
                concat_imgs[:, c, :, target.shape[3] + 2: target.shape[3] * 2 + 2] = \
                    pos_sample[:, c, ...] * VGG_M_2048.INPUT_STD[c] + VGG_M_2048.INPUT_MEAN[c]
                concat_imgs[:, c, :, target.shape[3] * 2 + 4:] = neg_sample[:, c, ...] * VGG_M_2048.INPUT_STD[c]\
                                                                 + VGG_M_2048.INPUT_MEAN[c]
...
def validate(val_loader, model, smoothness_criterion, bbox_criterion, cls_criterion, epoch, logger):
    ...
    for i, (target, pos_sample, neg_sample, cid, bbox, pos_bbox) in enumerate(val_loader):
        # Move the tensors to GPU.
        target = target.cuda(non_blocking=True)
        pos_sample = pos_sample.cuda(non_blocking=True)
        neg_sample = neg_sample.cuda(non_blocking=True)
    ...
    logger.scalar_summary('validation/cls_losses', cls_losses.avg, epoch)
    logger.scalar_summary('validation/bbox_losses', bbox_losses.avg, epoch)

    print(' * Smoothness losses {smoothness_loss:.3f}\n'
          ' * Classification losses {cls_loss:.3f}\n'
          ' * Bounding box losses {bbox_loss:.3f}'
          .format(smoothness_loss=(pos_losses.avg + neg_losses.avg),
